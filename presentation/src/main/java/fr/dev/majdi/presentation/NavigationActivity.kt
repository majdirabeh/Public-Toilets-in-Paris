...
import fr.dev.majdi.presentation.databinding.NavigationActivityBinding
 ...
 */
//I just implement this Activity from doc of mapbox to test navigation optional step i do it
...
// update top banner with maneuver instructions
        ...
        maneuvers.fold({ error ->
            Toast.makeText(
                this@NavigationActivity, error.errorMessage, Toast.LENGTH_SHORT
            ).show()
        }, {
...
// and later when a route is set also receiving route progress updates
    ...
    var toiletLatitude = 0.0
    var toiletLongitude = 0.0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.hasExtra("MyLocation") && intent.hasExtra("ToiletLocation")) {
            // Retrieve the double arrays from the Map Screen
            val myLocation = intent.getDoubleArrayExtra("MyLocation")
            val toiletLocation = intent.getDoubleArrayExtra("ToiletLocation")
            ...
            toiletLocation?.let {
                if (it.size == 2) {
                    toiletLatitude = it[1] ?: 0.0
                    toiletLongitude = it[0] ?: 0.0
                }
            }


            // Perform any necessary operations with the retrieved data
            // For example, you might want to display these values or use them in navigation logic
        } else {
            // Handle the case where the Intent doesn't have the expected extras
            Toast.makeText(this, "Missing data in Intent", Toast.LENGTH_SHORT).show()
...
// shows/hide the recenter button depending on the camera state
        ...
        speechApi = MapboxSpeechApi(
            this, getString(R.string.mapbox_access_token), Locale.US.language
        )
...
//                true
//            }
            val point = Point.fromLngLat(toiletLongitude, toiletLatitude)
            findRoute(point)
...
// comment out the location engine setting block to disable simulation
                .locationEngine(replayLocationEngine).build()
        )
        ...
        binding.mapView.location.apply {
            ...
            this.locationPuck = LocationPuck2D(
                bearingImage = ContextCompat.getDrawable(
                    this@NavigationActivity, R.drawable.mylocation
                )
            )
            enabled = true
        }
        ...
        mapboxReplayer.pushEvents(
            listOf(
                ReplayRouteMapper.mapToUpdateLocation(
                    Date().time.toDouble(),
                    Point.fromLngLat(myLongitude, myLatitude)
                )
            )
        )
